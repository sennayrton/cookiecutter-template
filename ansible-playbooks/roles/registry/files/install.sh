...
DIR="/usr/local/pr/kamino/registry"
...
workdir="/usr/local/pr/kamino/registry"
cd $workdir
...
then
    ...
    docker load -i /usr/local/pr/kamino/registry/harbor*.tar.gz
fi
echo ""
...
if [ -n "$host" ]
then
    sed "s/^hostname: .*/hostname: $host/g" -i /usr/local/pr/kamino/registry/harbor.yml
fi
